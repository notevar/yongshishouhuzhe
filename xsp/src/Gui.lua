...
function ui:new(width,height,okname,cancelname,config)
	...
	self["width"] 		= tonumber(width)*0.9 or w_*0.9
	self["height"] 		= tonumber(height)*0.9 or h_*0.9
...
function ui:rect_free(w,h,type)
	...
	elseif (type=="check") then
		return table.concat({math.floor(x),math.floor(y)+7+math.floor(0.03*ref_size),math.floor(w),math.floor(h)},",")
	elseif (type=="combox") then
		return table.concat({math.floor(x),math.floor(y)-11.8+math.floor(0.03*ref_size),math.floor(w),math.floor(h)},",")
	elseif (type=="radio") then
		return table.concat({math.floor(x),math.floor(y)+4.2+math.floor(0.03*ref_size),math.floor(w),math.floor(h)},",")
	elseif (type=="edit") then
		return table.concat({math.floor(x),math.floor(y)-9.2+math.floor(0.03*ref_size),math.floor(w),math.floor(h)},",")
	else
		return table.concat({math.floor(x),math.floor(y),math.floor(w),math.floor(h)},",")
	end
end
...
function ui:addRadioBoxGroup(index_w,index_h,id,def_value,...)--添加选择控件
	...
	local rect = self:rect_free(w,h,"radio")
	local tab = {...}
	local arr = {
		["type"] 	= "RadioGroup",
		["id"] 		= id,
...
function ui:addCheckBoxGroup(index_w,index_h,id,def_value,...)--添加选择控件
	local size = math.floor(0.35*ref_size)
	local w,h = math.floor(index_w*ref_width),math.floor(0.8*index_h*ref_size)
	local rect = self:rect_free(w,h,"check")
	--sysLog(rect)
	local tab = {...}
	local arr = {
		["type"] 	= "CheckBoxGroup",
...
function ui:addLebel(index_w,index_h,text,size,align,color,extra)
	...
	local rect = self:rect_free(w,h,"label")
	local arr = {
		["type"]    = "Label",
		["text"]    = text,
		["size"]    = size,
...
function ui:addComboBox(index_w,index_h,id,def_value,...)--添加选择控件
	...
	local rect = self:rect_free(w,h,"combox")
	local tab = {...}
	local arr = {
		["type"] 	= "ComboBox",
		["id"] 		= id,
...
function ui:addEdit(index_w,index_h,id,def_value,prompt,kbtype,size,align,color)--添加编辑框
	local w,h = math.floor(index_w*ref_width),math.floor(0.8*index_h*ref_size)
	if is_ios then h = math.floor(0.65*index_h*ref_size) end
	local rect = self:rect_free(w,h,"edit")
	local arr = {
		["type"] = "Edit",
		["id"] = id,
		["prompt"] = prompt or "",
...
local read = function(tmp,retTable)
	...
	for k,v in pairs(retTable) do
		...
		if tmp_k.CK then
			...
			for n = 1, # tab do
				local key = tmp_k[tab[n]+1]
				retTable[k] = retTable[k].."@"..tab[n]
			end
			retTable[k] = string.sub(retTable[k],2,-1)
